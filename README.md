...
用户命令 `/lol ...` 会先由 `main.py` 解析并分发到数据访问层 `fetcher/api.py`；访问层优先调用 PandaScore，必要时回退到 citoapi，并结合 TTL 缓存与统一结果封装。随后由 `formatter/message.py` 生成文本结果，必要时再由 `image_renderer.py` 输出图片，后台推送则由 `scheduler.py` 负责。
